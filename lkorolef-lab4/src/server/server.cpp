...
#define FILE_LOCK_FLAG 4 // file writing busy
...
namespace{
    void logPacketEvent(const std::string &type, uint32_t pkt_sn){
    ...
    bool write_window(Conn::ClientState* clientState){
        ...
        if(file_handle.file_write_stream(*clientState->fileStream, clientState->buffer) < 0){
            return false;
        } // TODO: maybe add safety if fileStream is nullptr (will cause segfault if it is)
...
void Server::server_recv(const int& droppc, const std::string& root_path){
    ...
    while(1){
        ...
        if(buffer[0] == META_FLAG){
            ...
            clientState->filePath = root_path + meta_data.second;
            ...
            if(!clientState->fileStream){
                ::write_busy(socket_p, client_addr, 0, addr_len);
                std::cout<<"FILE ALREADY BEING READ"<<std::endl; // debugging
            }
            continue;
        }
        ...
        } else if(clientState->base_seq_num > seq_num){ // from previous window
            ::server_send_ack(socket_p, client_addr, seq_num, addr_len, droppc);
        } else{
            clientState->buffer[seq_num] = std::vector<uint8_t>(buffer.begin() + 5, buffer.end());
            clientState->expected_seq_num += 1;
            ::server_send_ack(socket_p, client_addr, seq_num, addr_len, droppc);
        }
